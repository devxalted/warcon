<script lang="ts">
	...
	// given the read without the write. Server log is upstream's `audit.read`, which is stricter
	// than the `server.view` we had: operators lose it until that capability is added to the role.
